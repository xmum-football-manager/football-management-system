...
| FR-19 | Matches must follow a three-state lifecycle: Scheduled → Live → Finished | All | Must-have | 1 |
| FR-20 | Only the Organizer can trigger match state transitions (Scheduled → Live and Live → Finished) | Organizer | Must-have | 1 |
...
| FR-25 | Admin creates Organizer and Scorekeeper accounts by sending an email invite link; the invitee sets their own password | Admin | Must-have | 1 |
...
| FR-28 | Tournament basic info (name, dates, location, description) remains editable at any time; format and points system lock once the first match is Scheduled | Organizer | Must-have | 1 |
...
| NFR-03 | Score and schedule updates must propagate to participants without a manual refresh | Performance | Implemented via long polling — client fetches current state every 10 seconds via HTTP. No WebSocket dependency. |
...
### Real-time Strategy
Participants poll the API every 10 seconds via a `setInterval` HTTP fetch. On each tick the client GETs current match scores, standings, and match status — the UI diffs and updates in place. No WebSocket connection required. Max staleness is 10 seconds, which is acceptable for a live football score context.
...
FR in Phase 1: FR-01 through FR-16, FR-19 through FR-22, FR-25 through FR-28, FR-30, FR-31.

**Additions accepted during planning review:**
- WhatsApp share button on public tournament URL
- Scorekeeper email notification on match assignment
- QR code generator for tournament URL
- Supabase keep-alive cron (GitHub Actions, every 5 days)
- Custom SMTP via Resend free tier (required — Supabase default SMTP is 2 emails/day)
...
| WebSocket vs polling for real-time? | Long polling — HTTP GET every 10s via setInterval. No WebSocket, no Supabase Realtime dependency. Simpler, fully stateless. |
