...
import { getOrganizersForTournament } from '@/lib/db/roles'
...
export async function GET(req: NextRequest) {
  ...
  const roles = await getOrganizersForTournament(service, tournamentId)
  ...
  const result = roles.map(r => ({ user_id: r.user_id, email: userMap.get(r.user_id) ?? r.user_id }))
